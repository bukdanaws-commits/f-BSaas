...
export default function DisplayMonitorSection() {
  const [queue, setQueue] = useState<DisplayQueueItem[]>([
    ...
    { id: '1', participantId: 'p1', name: 'John Doe', company: 'Tech Corp', photoUrl: null, isDisplayed: false, createdAt: new Date().toISOString() },
    { id: '2', participantId: 'p2', name: 'Jane Smith', company: 'Design Studio', photoUrl: null, isDisplayed: false, createdAt: new Date().toISOString() },
  ])
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      setStats({
        ...
        checkedInParticipants: 45,
        totalClaims: 120,
        recentCheckIns: queue.map((q, i) => ({
          id: q.id,
          participantName: q.name,
          participantCompany: q.company,
          deskNumber: (i % 4) + 1,
                ...
                {queue.map((item, index) => (
                  ...
                  >
                    ...
                    <div className="text-right">
                      {index === 0 ? (
                        <Badge className="bg-[#47b2e4] text-white mb-2">Sedang Ditampilkan</Badge>
                      ) : (
                        <Badge variant="outline" className="mb-2">Menunggu</Badge>
                      )}
                    ...
                    {index === 0 && (
                      <Button variant="outline" size="sm" onClick={() => markAsDisplayed(item.id)}>
